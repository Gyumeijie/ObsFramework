...
static TD_ActionOutcome doAction(void *obj)
{
    ...
    unsigned int packetLength = fixedLength+dataLength + 
                                numberBlocks*(sizeof(TD_PUSMemData*) + 
                                sizeof(TD_PUSMemLength) + sizeof(unsigned short));

    // perform a consistency check: the number of blocks must be consistent with the
    ...
    for (TD_PUSNumberMemBlocks i=0; i<numberBlocks; i++) {
        ...
        memcpy(&startAddress, This->data+offset, sizeof(TD_PUSMemData*));
...
static void instance_init(Object *obj)
{
    ...
    tc->setSubType(obj, PUS_ST_TC_DMP_ABS);

    CC_RootObject_setClassId((CC_RootObject*)obj, ID_PUSDUMPMEMORYABSOLUTE);
}

DC_PUSDumpMemoryAbsolute* DC_PUSDumpMemoryAbsolute_new(void)
{
    Object *obj = object_new(TYPE_DC_PUSDUMPMEMORYABSOLUTE);
    return (DC_PUSDumpMemoryAbsolute*)obj;
}
...
static const TypeInfo type_info = {
    .name = TYPE_DC_PUSDUMPMEMORYABSOLUTE,
    .parent = TYPE_PUSDUMPMEMORY,
    .instance_size = sizeof(DC_PUSDumpMemoryAbsolute),
    .abstract = false,
    .class_size = sizeof(DC_PUSDumpMemoryAbsoluteClass),
