...
static void setRawDataFastly(void *obj, unsigned char *d, unsigned int n)
{
    ...
    DC_PUSMemoryLoadOffset *This = DC_PUSMEMORYLOADOFFSET(obj);
    ...
    memcpy(&parent->numberBlocks, 
           d + sizeof(TD_PUSMemId) + sizeof(TD_PUSMemData*),
    ...
    counter = sizeof(TD_PUSMemId) + sizeof(TD_PUSMemData*) + 
              sizeof(TD_PUSNumberMemBlocks);
    dataCounter = 0;
    ...
    for (unsigned int i=0; i<parent->numberBlocks; i++) {
        ...
        // For temporary use. 
        unsigned char *dataOffset = d + counter + sizeof(TD_PUSMemOffset) +
...
static void instance_init(Object *obj)
{
    ...
    CC_RootObject_setClassId((CC_RootObject*)obj, ID_PUSMEMORYLOADOFFSET);

    TelecommandClass *tc = TELECOMMAND_GET_CLASS(obj);
    tc->setSubType(obj, PUS_ST_TC_MEM_OFF);
}

DC_PUSMemoryLoadOffset* DC_PUSMemoryLoadOffset_new(void)
{
    Object *obj = object_new(TYPE_DC_PUSMEMORYLOADOFFSET);
    return (DC_PUSMemoryLoadOffset*)obj;
}
...
static const TypeInfo type_info = {
    .name = TYPE_DC_PUSMEMORYLOADOFFSET,
    .parent = TYPE_PUSMEMORYLOAD,
    .instance_size = sizeof(DC_PUSMemoryLoadOffset),
    .abstract = false,
    .class_size = sizeof(DC_PUSMemoryLoadOffsetClass),
